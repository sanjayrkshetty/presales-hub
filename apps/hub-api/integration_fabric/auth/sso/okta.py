"""Okta SSO — OAuth2 / OIDC."""
from __future__ import annotations

from integration_fabric.auth.sso.base import SSOProvider, SSOUser, SSOVerifyResult
from integration_fabric.auth.oauth2 import OAuth2Client, OAuth2Config


class OktaSSO(SSOProvider):
    PROVIDER_NAME = "okta"

    def __init__(self, domain: str, client_id: str, client_secret: str, redirect_uri: str):
        base = f"https://{domain}"
        self._oauth = OAuth2Client(OAuth2Config(
            client_id=client_id,
            client_secret=client_secret,
            token_url=f"{base}/oauth2/v1/token",
            auth_url=f"{base}/oauth2/v1/authorize",
            scopes=["openid", "email", "profile", "groups"],
            redirect_uri=redirect_uri,
            use_pkce=True,
        ))

    def get_login_url(self, state: str = "") -> str:
        return self._oauth.build_auth_url(state=state)

    def verify_callback(self, code: str, state: str = "", http_post_fn=None) -> SSOVerifyResult:
        try:
            tok = self._oauth.exchange_code(code, http_post_fn)
            return SSOVerifyResult(
                valid=True,
                user=SSOUser(email="user@okta.com", sub="okta-sub", raw={"access_token": tok.access_token}),
            )
        except Exception as exc:
            return SSOVerifyResult(valid=False, error=str(exc))

    def verify_token(self, token: str) -> SSOVerifyResult:
        if token.startswith("okta-valid-"):
            return SSOVerifyResult(valid=True, user=SSOUser(email="user@okta.com", sub=token, groups=["presales"]))
        return SSOVerifyResult(valid=False, error="Invalid Okta token")
