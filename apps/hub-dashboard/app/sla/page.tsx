        ...
        <div className="panel">
          ...
          <div className="p-3 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {breached.slice(0, 6).map((opp) => (
              <Link key={opp.opportunity_id} href={`/proposals/${opp.proposal_id ?? opp.opportunity_id}`}
                ...
                <span className="text-2xs text-text-muted font-sans">{opp.client_name} · {opp.stage.replace(/_/g, " ")}</span>
                ...
                <div className="health-bar mt-1">
                  <div className="health-bar-fill bg-danger" style={{ width: `${Math.min(100, Math.abs(opp.hours_remaining) / opp.hours_allowed * 100)}%` }} />
                </div>
              </Link>
            ))}
          </div>
      ...
      {warning.length > 0 && (
        <div className="panel">
          ...
          <table className="data-table">
            ...
            <tbody>
              {warning.map((opp) => {
                const pctUsed = Math.round((1 - opp.hours_remaining / opp.hours_allowed) * 100);
                return (
                  <tr key={opp.opportunity_id}>
                    <td>
                      <Link href={`/proposals/${opp.proposal_id ?? opp.opportunity_id}`}
                    ...
                    <td><span className="font-mono text-xs text-warn">{formatHours(opp.hours_remaining)}</span></td>
                    <td>
